...
//Rare exceptions to indicate a bug in the _nokundaone code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_nokundaone = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_nokundaone]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.nokundaone.module"
